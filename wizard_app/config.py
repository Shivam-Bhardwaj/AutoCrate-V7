...
SKID_DIMENSIONS: dict = { "3x4": (2.5, 3.5), "4x4": (3.5, 3.5), "4x6": (5.5, 3.5) } # Actual Width, Actual Height
WEIGHT_RULES: list = [ 
    (500,"3x4",30.0), 
    (4500,"4x4",30.0), 
    (6000,"4x6",41.0), 
    (12000,"4x6",28.0), 
    (20000,"4x6",24.0) 
] # (MaxProductWeight, SkidNominalType, MaxSkidCenterToCenterSpacing)
MIN_SKID_HEIGHT: float = 3.5
SKID_LENGTH_ASSUMPTION: str = "crate_overall_length" # Skids run the full length of the crate

# --- Floorboard Logic Constants ---
ALL_STANDARD_FLOORBOARDS: dict = { 
    "2x6": 5.5, # Nominal : Actual Width
    "2x8": 7.25, 
    "2x10": 9.25, 
    "2x12": 11.25 
}
STANDARD_FLOORBOARD_LUMBER_ACTUAL_THICKNESS: float = 1.5 # Actual thickness for 2x lumber
MIN_CUSTOM_NARROW_WIDTH: float = 2.50 # Minimum width for a custom fill piece
MAX_CUSTOM_NARROW_WIDTH: float = 5.50 # Maximum width for a custom fill piece (before it should be a std size)
EFFECTIVE_MAX_CUSTOM_WIDTH = MAX_CUSTOM_NARROW_WIDTH - (FLOAT_TOLERANCE * 10)
MIN_STANDARD_BOARD_WIDTH: float = 5.50 # Smallest standard board actual width we prefer to use
MAX_STANDARD_BOARD_WIDTH: float = 11.25 # Largest standard board actual width
MAX_CENTER_GAP: float = 0.25 # Max allowed gap in the center of the floorboard layout

# --- Cap & Wall Logic Constants ---
DEFAULT_CLEAT_NOMINAL_THICKNESS: float = 0.75 # Actual thickness
DEFAULT_CLEAT_NOMINAL_WIDTH: float = 3.5    # Actual width
WALL_PLYWOOD_THICKNESS_MIN: float = 0.25
DEFAULT_WALL_PLYWOOD_THICKNESS: float = 0.25
INTERMEDIATE_CLEAT_THRESHOLD: float = 48.0 # If panel span exceeds this, intermediate cleats might be needed
MAX_INTERMEDIATE_CLEAT_SPACING = 24.0 # Specific max spacing for intermediate cleats
PLYWOOD_STD_WIDTH = 48.0 # Standard plywood sheet width
PLYWOOD_STD_HEIGHT = 96.0 # Standard plywood sheet height
...
DEFAULT_STANDARD_LUMBER_NOMINALS_UI: list = [ 
    k for k, v in ALL_STANDARD_FLOORBOARDS.items() 
    if (v >= MIN_STANDARD_BOARD_WIDTH - FLOAT_TOLERANCE and 
        v <= MAX_STANDARD_BOARD_WIDTH + FLOAT_TOLERANCE) 
]
...
DECAL_RULES = {
    ...
    "handling_horizontal": {
        ...
        "text_content": "↑☂↔↑", # Placeholder for handling symbols (using ↔ for "this way up" arrows)
...
# --- Visualization Colors & Styles (Primarily for any internal visualization, not directly for NX) ---
...
SKID_COLOR_VIZ: str = "#8B4513"; SKID_OUTLINE_VIZ: str = "#654321"
FLOORBOARD_STD_COLOR_VIZ: str = "#D2B48C"; FLOORBOARD_CUSTOM_COLOR_VIZ: str = "#B0C4DE"; FLOORBOARD_OUTLINE_VIZ: str = "#4682B4"
